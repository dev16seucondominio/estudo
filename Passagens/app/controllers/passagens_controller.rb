class PassagensController < ApplicationController
  ...
  def save
    status, resp = PassagensService.save(params_pagador)
  ...
  def destroy
    status, resp = PassagensService.destroy(params)
  ...
  def show
    status, resp = PassagensService.show(get_params)
  ...
  def params_pagador
    attrs = [:pagador, :id, :tipo, :juridica, :sexo, :deficiente, :nome, :doc, :rg, :nasc, :prof, :email,
